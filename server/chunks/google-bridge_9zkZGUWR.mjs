...
function asGrid(v) {
  if (typeof v === "string") {
    const s = v.replace(/^```(?:json)?\s*|\s*```$/gi, "").trim();
    if (!s) return [];
    try {
      const p = JSON.parse(s);
      if (Array.isArray(p)) return asGrid(p);
    } catch {
    }
    return [[s]];
  }
...
const gridEmpty = (g) => g.length === 0 || g.every((row) => row.length === 0 || row.every((c) => c == null || String(c).trim() === ""));
...
${e.htmlLink}` : ""}`;
    ...
    case "google.sheets.append": {
      const grid = asGrid(a.values);
      if (gridEmpty(grid)) return "（追記する内容が空のためスキップしました）";
      const r = await g(`https://sheets.googleapis.com/v4/spreadsheets/${enc(extractGoogleId(asStr(a.id)))}/values/${enc(asStr(a.range) || "A1")}:append?valueInputOption=USER_ENTERED&insertDataOption=INSERT_ROWS`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ values: grid }) });
      if (!r) return NEED;
    ...
    case "google.sheets.update": {
      const grid = asGrid(a.values);
      if (gridEmpty(grid)) return "（更新する内容が空のためスキップしました）";
      const r = await g(`https://sheets.googleapis.com/v4/spreadsheets/${enc(extractGoogleId(asStr(a.id)))}/values/${enc(asStr(a.range))}?valueInputOption=USER_ENTERED`, { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ values: grid }) });
      if (!r) return NEED;
