...
import { saveApiKey, getApiKey, deleteApiKey } from "./client_Ceeu_qGY.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { clearServiceAccount, serviceAccountConfigured, serviceAccountAccessToken } from "./google-sa_BQsetpcp.mjs";
const SCOPE_GROUPS = {
  calendar: {
...
const SCOPES_KEY_SA = "google_scopes_sa";
...
async function setGoogleGroups(env, groups) {
  await kvPut(env, SCOPES_KEY_SA, JSON.stringify(normalizeGroups(groups)));
...
async function grantedGroups(env) {
  const key = await serviceAccountConfigured(env) ? SCOPES_KEY_SA : SCOPES_KEY;
  try {
    const arr = JSON.parse(await env.LICENSE.get(key) ?? "[]");
    return (Array.isArray(arr) ? arr : []).filter((g) => g in SCOPE_GROUPS);
  } catch {
    return [];
  }
}
async function resetGoogleAccountData(env) {
  await env.DB.prepare("DELETE FROM app_event_cursors").run().catch(() => {
  });
  await env.DB.prepare("DELETE FROM drive_files").run().catch(() => {
  });
  await env.DB.prepare("UPDATE schedules SET google_event_id=NULL, google_synced_at=NULL WHERE google_event_id IS NOT NULL").run().catch(() => {
  });
}
async function exchangeGoogleCode(env, origin, code, groups) {
  ...
  if (t.access_token) {
    const info = await fetch("https://www.googleapis.com/oauth2/v2/userinfo", { headers: { authorization: `Bearer ${t.access_token}` } }).then((x) => x.ok ? x.json() : null).catch(() => null);
    const acct = info?.id || info?.email || "";
    if (acct) {
      const prev = await env.LICENSE.get(ACCOUNT_KEY).catch(() => null);
      if (prev && prev !== acct) await resetGoogleAccountData(env);
      await kvPut(env, ACCOUNT_KEY, acct);
    }
  }
...
async function disconnectGoogle(env) {
  ...
  await (await import("./drive_CKlNA6wn.mjs")).disconnectDrive(env).catch(() => {
  });
