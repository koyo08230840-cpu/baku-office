...
import { APP_SCHEMA, validateDefinition } from "./appdef_CnTZd11a.mjs";
import { ALLOWED_PERMISSIONS, PRIVILEGED_PERMISSIONS } from "./apps_BKcWc3Sq.mjs";
import { p as preflight } from "./preflight_d8LingHU.mjs";
...
const DESTRUCTIVE_SQL = /\b(drop\s+table|delete\s+from|truncate\s+table|alter\s+table|attach\s+database|pragma\b|update\s+\w+\s+set(?![^;]*\bwhere\b))/i;
function collectRawSql(def) {
  const out = [];
  const d = def;
  const push = (steps) => {
    ...
    for (const s of steps) {
      const st = s;
      if (st && (st.op === "db.query" || st.op === "db.write") && typeof st.sql === "string") out.push(st.sql);
    }
  };
  push(d?.steps);
...
async function deterministicSelfChecks(ctx, input) {
  ...
  const html = def && typeof def === "object" && typeof def.render?.html === "string" ? def.render.html : "";
  const badSql = collectRawSql(def).some((q) => DESTRUCTIVE_SQL.test(q));
...
async function scanLeakedSecrets(ctx, text) {
  const { listApiKeyNames, getApiKey } = await import("./client_Ceeu_qGY.mjs");
