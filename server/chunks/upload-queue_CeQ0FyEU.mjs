...
async function interpretInstruction(env, text) {
  ...
  try {
    const { parseJsonObject } = await import("./ctx_CknKdTPU.mjs").then((n) => n.V);
...
async function processOwnerBatch(ctx, owner, baseUrl) {
  const now = nowSec();
  const claim = await ctx.db.run("UPDATE upload_queue SET status='processing', attempts=attempts+1, lease_expiry=?, updated_at=? WHERE owner=? AND status='queued' AND mode IS NOT NULL", [now + UPLOAD_LEASE, now, owner]);
  ...
  let effRole = rows.find((r) => r.role)?.role ?? "member";
  let principalUid = owner;
  const om = owner.match(/^([a-z]+):(.+)$/);
  if (om) {
    const { localIdentity } = await import("./ctx_CknKdTPU.mjs").then((n) => n.Y);
    const u = await localIdentity(ctx).memberOf(om[1], om[2]).catch(() => null);
    if (u && u.status !== "active") {
      for (const r of rows) await ctx.db.run("UPDATE upload_queue SET status='error', last_error=?, lease_expiry=NULL, updated_at=? WHERE id=?", ["principal_suspended", now, r.id]);
      return { processed: 0 };
    }
    if (u) {
      effRole = u.role;
  ...
  const parts = [];
  if (mode === "skip") {
    const { softDeleteFile } = await import("./storage_BN4XcC1w.mjs");
...
一覧：${baseUrl}/files`;
    ...
    for (let i = 0; i < rows.length; i += CONC) {
      ...
      const res = await Promise.all(chunk.map((r) => readOne(ctx, r, context, baseUrl, effRole, principalUid).catch((e) => `読み取りに失敗しました（${e.message}）。`)));
      chunk.forEach((r, j) => {
        outs[i + j] = `【${label(r)}】
${res[j]}`;
      });
  ...
  const finalStatus = mode === "skip" ? "skipped" : "done";
  for (let i = 0; i < rows.length; i++) {
    await ctx.db.run(
      "UPDATE upload_queue SET status=?, result=?, lease_expiry=NULL, notified=0, updated_at=? WHERE id=?",
      [finalStatus, i === 0 ? text : null, now, rows[i].id]
    );
  }
  const sent = await notifyOwnerDirect(ctx, owner, text).then((n) => n > 0).catch(() => false);
  if (sent) await ctx.db.run("UPDATE upload_queue SET notified=1 WHERE owner=? AND result IS NOT NULL AND status IN ('done','skipped')", [owner]).catch(() => {
  });
  else await ctx.db.run("UPDATE upload_queue SET last_error=? WHERE owner=? AND status=? AND result IS NOT NULL", ["notify_failed", owner, finalStatus]).catch(() => {
  });
  return { processed: total };
}
async function readOne(ctx, r, context, baseUrl, effRole, principalUid) {
  const f = await getFile(ctx.env, r.file_id);
  if (!f) return "ファイルを取得できませんでした。";
  const mime = (r.mime || f.mime || "").toLowerCase();
  if (mime.startsWith("audio/")) {
    const { transcribeAudio } = await import("./ctx_CknKdTPU.mjs").then((n) => n.Q);
  ...
  if (mime.startsWith("image/")) {
    ...
    const ans = await ctx.agent.run({ owner: principalUid ?? r.owner, text: prompt, image: { mimeType: f.mime, dataB64: b64(f.buf) }, role: (effRole ?? r.role) || "member", baseUrl, sessionId: r.owner }).catch((e) => `読み取りに失敗しました（${e.message}）。`);
...
async function recoverStaleUploadBatches(ctx) {
  const now = nowSec();
  const stale = await ctx.db.all(
    "SELECT id,owner,attempts FROM upload_queue WHERE status='processing' AND (lease_expiry IS NULL OR lease_expiry < ?)",
    [now]
  );
  let recovered = 0;
  const deadOwners = /* @__PURE__ */ new Set();
  for (const r of stale) {
    if ((r.attempts ?? 0) >= UPLOAD_MAX_ATTEMPTS) {
      await ctx.db.run("UPDATE upload_queue SET status='error', last_error=?, lease_expiry=NULL, updated_at=? WHERE id=? AND status='processing'", ["max_attempts", now, r.id]);
      deadOwners.add(r.owner);
    } else {
      const back = await ctx.db.run("UPDATE upload_queue SET status='queued', lease_expiry=NULL, updated_at=? WHERE id=? AND status='processing' AND (lease_expiry IS NULL OR lease_expiry < ?)", [now, r.id, now]);
      if (back.rowsWritten) recovered++;
    }
  }
  for (const owner of deadOwners) await notifyOwnerDirect(ctx, owner, "⚠️ 送っていただいたファイルの処理が完了できませんでした。お手数ですが、もう一度お送りください。").catch(() => 0);
  return recovered;
}
async function processReadyUploads(ctx, baseUrl, limit = 5) {
  let n = 0;
  await recoverStaleUploadBatches(ctx).catch(() => 0);
