...
async function getMessages(ctx, sessionId) {
  const rows = await ctx.db.all(
    "SELECT id,role,content,created_at,actions FROM (SELECT id,role,content,created_at,actions,rowid AS rid FROM chat_messages WHERE session_id=? ORDER BY created_at DESC, rowid DESC LIMIT 200) ORDER BY created_at ASC, rid ASC",
    [sessionId]
  );
...
async function saveMessageAttachments(ctx, messageId, sessionId, owner, atts) {
  if (!atts.length) return;
  const now = nowSec();
  await ctx.db.batch(atts.slice(0, 8).map((a) => ({
    sql: "INSERT INTO message_attachments (id,message_id,session_id,owner,file_id,mime,name,summary,created_at) VALUES (?,?,?,?,?,?,?,?,?)",
    params: [randomId(), messageId, sessionId, owner, a.fileId, a.mime ?? null, a.name ?? null, (a.summary ?? "").slice(0, 500) || null, now]
  }))).catch(() => {
  });
}
async function getSessionAttachments(ctx, sessionId, limit = 8) {
  return await ctx.db.all(
    "SELECT file_id AS fileId, mime, name, summary FROM message_attachments WHERE session_id=? ORDER BY created_at DESC, id DESC LIMIT ?",
    [sessionId, limit]
  ).catch(() => []);
}
...
export {
  ...
  getMessages as g,
  getSessionAttachments as h,
  deleteSession as i,
  estimateToolTokens as j,
  chatActions as k,
  listSessions as l,
  chatSessions as m,
  navGuidance as n,
