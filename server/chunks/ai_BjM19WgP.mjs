...
async function runToolLoop(model, system, first, tools, exec, maxHops = 4, priorHistory = [], onUsage, abort, onEvent, answerMaxTokens, cacheBoundary, parallelSafe) {
  ...
  for (let h = 0; h < maxHops; h++) {
    ...
    const allParallelSafe = calls.length > 1 && !!parallelSafe && calls.every((c) => parallelSafe(c.name));
    let results;
    if (allParallelSafe) {
      results = await Promise.all(calls.map(async (c) => ({ id: c.id, name: c.name, content: await exec(c.name, c.args) })));
    } else {
      results = [];
      for (const c of calls) results.push({ id: c.id, name: c.name, content: await exec(c.name, c.args) });
    }
