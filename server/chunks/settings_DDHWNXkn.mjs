...
import { setMaxUploadMb, setRetentionDays } from "./storage_BN4XcC1w.mjs";
...
import { setAutonomy, saveAutonomyConfig } from "./autonomy_DO1Z-v4Z.mjs";
import { setStorageLimits } from "./storage-usage_Dtj1S6Ue.mjs";
...
import { installApp, uninstallApp, installedAppIds, appCatalog } from "./apps_BKcWc3Sq.mjs";
import { f as fetchAndInstall, u as uninstallExternal, b as listExternalApps, c as forkExternalApp, l as listDrafts, s as submitDraft, p as proposeUpstreamMerge, d as applyUpstreamMerge, e as deleteDraft, h as purgeStaleDrafts, i as installLocalApp, a as activeAppDefinition, j as appVersions, k as activateAppVersion, m as deleteLocalApp } from "./external-apps_DRfaI5xV.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  if (b._action === "agent_approval") {
    const { setApprovalMode } = await import("./approvals_DakhuEZU.mjs");
  ...
  if (b._action === "notify_webhook_test") {
    ...
    const { pushWebhook, notifyHook } = await import("./notifications_bYzhELs1.mjs");
    let sent = false;
    let okHost = false;
    try {
    ...
    if (okHost) {
      const { cfEgressGateway } = await import("./ctx_CknKdTPU.mjs").then((n) => n.N);
      try {
  ...
  if (b._action === "notify_line_test") {
    const { notifyOwnerDirect } = await import("./ctx_CknKdTPU.mjs").then((n2) => n2.T);
  ...
  if (b._action === "cf_provision") {
    ...
    const { buildCfClient, provisionCaseA } = await import("./cf-provision_qlOHYsZ8.mjs");
  ...
  if (b._action === "app_dev_log") {
    ...
    const { getMessages } = await import("./chat-sessions_BfrxKJEd.mjs").then((n) => n.m);
  ...
  if (b._action === "allow_relaxed_host") {
    ...
    const { enableRelaxedForApp } = await import("./external-apps_DRfaI5xV.mjs").then((n) => n.D);
  ...
  if (b._action === "set_app_meta") {
    ...
    const { setAppMeta, getAppMeta } = await import("./external-apps_DRfaI5xV.mjs").then((n) => n.C);
  ...
  if (b._action === "set_app_roles") {
    ...
    const { setAppAllowedRoles } = await import("./external-apps_DRfaI5xV.mjs").then((n) => n.D);
  ...
  if (b._action === "ai_retag") {
    const id = String(b.appId ?? b.draftId ?? "").trim();
    if (!id) return json({ error: "対象がありません" }, 400);
    const { getAppDesign } = await import("./external-apps_DRfaI5xV.mjs").then((n) => n.D);
    const { suggestAppMeta, setAppMeta } = await import("./external-apps_DRfaI5xV.mjs").then((n) => n.C);
