...
import { $ as $$App } from "./App_DGe4pk2V.mjs";
...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_Ceeu_qGY.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_DN9VKrrA.mjs");
  await import("./index_D48l-BFG.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_BKcWc3Sq.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_Dtj1S6Ue.mjs");
  ...
  const { reviewQueue } = await import("./users_6-7ybvKH.mjs");
  const { backupAlert } = await import("./backup_TC22fx1l.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_DRfaI5xV.mjs").then((n) => n.D);
  ...
  const { storageMode } = await import("./storage_BN4XcC1w.mjs");
