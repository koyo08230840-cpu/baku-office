...
const $$Domain = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const entitlement = await (await import("./client_Ceeu_qGY.mjs")).cachedEntitlement(env);
  ...
  const pubPages = cfg ? await (await import("./public-pages_C8bkA-DV.mjs")).listPublicPages(env) : [];
  const defPublic = cfg?.publicHost || cfg?.domain || "";
  const defAdmin = cfg?.adminHost || (cfg?.domain ? "app." + cfg.domain : "");
  const cfTokenReady = cfg ? await (await import("./autonomy_DO1Z-v4Z.mjs")).getAutonomyConfig(env).then((a) => a.cfToken).catch(() => false) : false;
