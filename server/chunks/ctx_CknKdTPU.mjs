...
import { a as atLeast, p as planLabel, r as roleLabel, i as isDeveloperRole, E as ENTITLEMENT_RANK, R as ROLE_LABELS } from "./types_BVJxqWI9.mjs";
...
import { getFile, saveFile, setRetentionDays, setMaxUploadMb, storageMode, fileBelongsTo } from "./storage_BN4XcC1w.mjs";
import { getApiKey, getToken, hostFetch, cachedEntitlement, entitlementForGate, masterKeyCtx } from "./client_Ceeu_qGY.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { addNotification, notifyHook } from "./notifications_bYzhELs1.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers, verifyPassword } from "./users_6-7ybvKH.mjs";
import { detectProfile } from "./profiles_D3vLhBYo.mjs";
import { permissionCatalogText, ALLOWED_PERMISSIONS, installedAppIds, appCatalog, installApp, uninstallApp, makeAppsApi } from "./apps_BKcWc3Sq.mjs";
import "./index_D48l-BFG.mjs";
...
import { googleFetch } from "./google_cIMnQJbU.mjs";
...
import { APP_SCHEMA, RENDER_HTML_MAX, collectionsNeedingWriter, validateDefinition, checkRenderScripts, checkRenderScriptsParsed, checkRenderBusReachability, checkRenderHandlers, checkRenderDomRefs, checkRenderDataKeys, checkRenderScreens, reconcileRenderScreenRefs, reconcileDataCollections, checkDataCollections, reconcileWriteFormToReadScreen, checkWriteScreenEmptyBoRun, checkWriteScreenArgKeyMismatch, checkReadOnlyCollections, unwiredRenderWriteScreens, opCatalogText, googleOpCatalogText } from "./appdef_CnTZd11a.mjs";
import { turnMedia, runToolLoop, HOPS_EXCEEDED, classifyModelError } from "./ai_BjM19WgP.mjs";
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_CrMfKBKu.mjs";
import { getOrgProfile, setOrgProfile, listPublicActions, listActions, createAction, deleteAction, updateAction } from "./a2a-actions_7tBc7piP.mjs";
import { brandName, getTheme } from "./theme_DFty9gzU.mjs";
import { setAutonomy, getAutonomyConfig, autonomyReady, AUTONOMY_TOOLS, AUTONOMY_POLICY, runAutonomyTool } from "./autonomy_DO1Z-v4Z.mjs";
import { i as runApp, p as platformInvariantSuspected, e as escalatePlatformInvariant, g as runInstalledApp, r as runDraftApp, f as appRunNeedsApproval, b as authorizeAppRun, j as googleToolPreflight } from "./app-runtime_DDtJx-OO.mjs";
...
import { i as isDisplayOnly, u as unitsOf, a as checkTransitions, s as sampleInputs } from "./precheck_BsGxECCD.mjs";
import { rebuildPublicPageFromDef, fieldsFromDefinition, renderFormHtml, upsertPublicPage, collectionFromDefinition } from "./public-pages_C8bkA-DV.mjs";
import { g as getAppDesign, n as appsBrief, o as createDraft, q as repairOutputLiterals, t as getAppMeta, v as setAppMeta, w as suggestAppMeta, i as installLocalApp, x as pendingRelaxedHosts, y as isRegisteredPart, f as fetchAndInstall, z as setAppAllowedRoles, b as listExternalApps, A as parseTags, B as installedAppDefs } from "./external-apps_DRfaI5xV.mjs";
import { a as appendMessage, b as estimateTokens, j as estimateToolTokens, p as providerInputCap } from "./chat-sessions_BfrxKJEd.mjs";
import { explainStop } from "./errors_Cz86HmdL.mjs";
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_B4vWtV1j.mjs";
import { storeCatalog } from "./store_DfdaLbR9.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_CcY7RZcf.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_DakhuEZU.mjs";
import { saveLayoutDraft, listSites } from "./sites_D1KDcV1z.mjs";
...
import { p as pushToUser } from "./push_DJQA1W_s.mjs";
...
async function lineReply(gw, accessToken, replyToken, text) {
  const r = await gw.fetch("line", "https://api.line.me/v2/bot/message/reply", {
    method: "POST",
...
async function linePush(gw, accessToken, to, text) {
  const r = await gw.fetch("line", "https://api.line.me/v2/bot/message/push", {
    method: "POST",
    headers: { "content-type": "application/json", authorization: `Bearer ${accessToken}` },
...
async function sendDirect(ctx2, gw, connector, externalId, text) {
  if (connector === "line") {
    ...
    return (await linePush(gw, token, externalId, text)).ok;
  }
...
function normalizePlan(raw) {
  ...
  const renderPhases = phases.filter((p) => p.kind === "render");
  const screenPhases = phases.filter((p) => p.kind === "screen");
  const screenOverflow = screenPhases.slice(6).map((p) => p.title).filter(Boolean);
  if (screenOverflow.length) followUps = [...followUps, ...screenOverflow].slice(0, 10);
  phases = [...screenPhases.slice(0, 6), ...renderPhases];
  if (isCustomUI && renderPhases.length === 0) phases.push({ title: "カスタムUI（画面の見た目）", goal: ((goal || name || "アプリ") + " のカスタムUI（画面レイアウト・操作）を描画").slice(0, 300), kind: "render", status: "todo" });
...
async function startAppBuild(ctx2, a) {
  if (a.sessionId) {
    ...
    if (dup?.id) {
      ...
      await kickBuild(ctx2, dup.id, a.baseUrl ?? "", "start-build").catch(() => {
      });
      return dup.id;
    }
  }
  ...
  await kickBuild(ctx2, id, a.baseUrl ?? "", "start-build").catch(() => {
  });
  return id;
}
function hashStr(s) {
...
async function startAppEdit(ctx2, a) {
  ...
  const dup = await ctx2.db.first(
    "SELECT id, spec FROM app_builds WHERE owner=? AND app_id=? AND kind='edit' AND status IN ('planning','building') AND created_at > ? ORDER BY created_at DESC LIMIT 1",
    [a.owner, a.appId, nowSec() - 90]
  ).catch(() => null);
  if (dup?.id && norm(dup.spec ?? "") === norm(a.instruction.slice(0, 16e3))) {
    await logBuild(ctx2.env, dup.id, "二重編集を抑止", "同appの進行中編集(同一指示)を再利用（新規生成せず）", "warn");
    await kickBuild(ctx2, dup.id, a.baseUrl ?? "", "start-edit").catch(() => {
    });
    return dup.id;
  }
  const id = "e" + Math.abs(hashStr(a.owner + a.appId + a.instruction + nowSec())).toString(36) + nowSec().toString(36);
  const now = nowSec();
  await ctx2.db.run(
  ...
  await kickBuild(ctx2, id, a.baseUrl ?? "", "start-edit").catch(() => {
  });
  return id;
}
...
async function repairDeadListCollections(ctx2, row, def, plan, model) {
  ...
  try {
    ...
    for (const rawCol of deadCol) {
      if (!await touchBuildLease(ctx2, row.id)) break;
...
検証エラー：${issueText}`;
  ...
  try {
    ...
    await markBuildErrorCas(ctx2, row.id, "draft_failed", msg);
    await post(ctx2, row, msg, true);
    return false;
  }
  let applied = false;
...
外部のAI（ChatGPT / Claude 等）で作る場合は、下の「📋 外部AI用プロンプトをコピー」を押して貼り付けてください。` : "");
      ...
      await markBuildErrorCas(ctx2, row.id, "infeasible", msg);
...
できる範囲（${feas.feasibleScope || "可能な部分"}）を作ります。作れない部分は、下の「📋 外部AI用プロンプトをコピー」で外部AI（ChatGPT 等）に依頼できます。`;
    ...
    const wonPlan = await ctx2.db.run(
      ...
      [plan2.name, JSON.stringify(plan2), JSON.stringify(def2), plan2.id, nowSec(), row.id]
    );
    if (!wonPlan.rowsWritten) return false;
    row.status = "building";
  ...
  const adv = await ctx2.db.run(
    ...
    [JSON.stringify(plan), JSON.stringify(def), row.cursor + 1, nowSec(), row.id, row.cursor]
  );
  if (!adv.rowsWritten) return false;
...
async function touchBuildLease(ctx2, id) {
  const r = await ctx2.db.run("UPDATE app_builds SET updated_at=? WHERE id=? AND status IN ('building','finalizing')", [nowSec(), id]).catch(() => ({ rowsWritten: 0 }));
  return !!r.rowsWritten;
}
async function markBuildErrorCas(ctx2, id, reason, msg) {
  const r = await ctx2.db.run(
    "UPDATE app_builds SET status='error', stop_reason=?, error=?, updated_at=? WHERE id=? AND status NOT IN ('error','done','done_partial','cancelled')",
    [reason, msg.slice(0, 500), nowSec(), id]
  );
  return !!r.rowsWritten;
}
async function failBuild(ctx2, row, msg, reason = "system") {
  const won = await markBuildErrorCas(ctx2, row.id, reason, msg);
  if (!won) return;
...
async function salvagePartial(ctx2, row, reason) {
  ...
  try {
    ...
    await markBuildErrorCas(ctx2, row.id, "draft_failed", "ここまでの実装はできましたが保存に失敗しました。").catch(() => {
    });
...
async function planSiteSections(env, opts) {
  if (opts.base?.blocks?.length) {
    const diff = await planSiteEdit(env, opts);
    if (diff.ok) return diff;
    return { ok: true, sections: opts.base.blocks.map((b, i) => ({ type: b.type, goal: "現状のまま保持", action: "keep", baseIndex: i })) };
  }
  const baseHint = "";
...
async function planSiteEdit(env, opts) {
  const constitutionGuard = opts.constitution ? opts.constitution.trim() + "\n\n【上のルールを前提に最小差分で編集する】\n" : "";
  const list = opts.base.blocks.map((b, i) => {
    const txt = Object.values(b.props).filter((v) => typeof v === "string" && v.trim()).map((v) => String(v)).join(" / ").slice(0, 80);
    return `#${i} ${b.type}${txt ? "：" + txt : ""}`;
  }).join("\n");
  const sys = constitutionGuard + `あなたはHP/LPの編集プランナーです。利用者の依頼に対し「最小差分」で編集計画を作ります。使えるブロック型：${ALLOWED_TYPES.join(", ")}。
【原則】依頼で明示的に変更が必要なブロックだけを update/add/削除し、それ以外は必ず keep（現状維持）する。指示に無いブロックを勝手に書き換え・削除しない。
【出力】最終的なページの並び順で、各セクションを1件ずつ JSON 配列で返す（前置き・説明・コードフェンス無し）。要素：
- 既存を残す： {"action":"keep","baseIndex":<元の#番号>}
- 既存を直す： {"action":"update","baseIndex":<元の#番号>,"goal":"何をどう直すか1文"}
- 新規追加：   {"action":"add","type":"<ブロック型>","goal":"何を伝えるか1文","priority":"P1|P2|P3"}
削除は「配列に含めない」ことで表現する（既存 index を1つだけ省く）。並べ替えは配列の順序で表現する。
依頼が特定ブロックのみの変更なら、他は全て keep にすること（byte-for-byte 保持されます）。`;
  const user = `【現在のブロック（#=位置）】
${list}

【依頼】
${opts.prompt.trim() || "全体を洗練する"}`;
  let raw = "";
  try {
    raw = await inferApp(env, user, { system: sys, maxTokens: 1500, modelId: opts.modelId, feature: "site_builder" });
  } catch {
    return { ok: false, error: "AI呼び出し失敗" };
  }
  const arr2 = parseJsonArray(raw);
  if (!arr2) return { ok: false, error: "差分プランを生成できませんでした" };
  const sections = parseSiteEditPlan(arr2, opts.base.blocks.map((b) => b.type));
  if (!sections.length) return { ok: false, error: "有効な差分がありません" };
  return { ok: true, sections };
}
function parseSiteEditPlan(arr2, baseTypes) {
  const n = baseTypes.length;
  const sections = [];
  for (const x of arr2) {
    if (!x || typeof x !== "object") continue;
    const o = x;
    const goal = String(o.goal ?? "").slice(0, 120);
    const priority = o.priority === "P1" || o.priority === "P2" || o.priority === "P3" ? o.priority : void 0;
    const idx = Number(o.baseIndex);
    const idxOk = Number.isInteger(idx) && idx >= 0 && idx < n;
    if (o.action === "add" && ALLOWED_TYPES.includes(String(o.type ?? ""))) {
      sections.push({ type: String(o.type), goal: goal || "新規セクション", action: "add", ...priority ? { priority } : {} });
    } else if (o.action === "update" && idxOk) {
      sections.push({ type: baseTypes[idx], goal: goal || "内容を改善", action: "update", baseIndex: idx });
    } else if (idxOk) {
      sections.push({ type: baseTypes[idx], goal: "現状のまま保持", action: "keep", baseIndex: idx });
    }
  }
...
function statusFromRow(row) {
  ...
  return { status: row.status, phaseLabel, cursor: row.cursor, total, note: row.note, error: row.error, slug: row.slug };
}
async function siteBuildStatus(ctx2, buildId, owner) {
  const row = await ctx2.db.first(`SELECT ${COLS} FROM site_builds WHERE id=? AND owner=?`, [buildId, owner]);
  return row ? statusFromRow(row) : null;
}
async function latestSiteBuildStatus(ctx2, slug, owner) {
  const row = await ctx2.db.first(`SELECT ${COLS} FROM site_builds WHERE slug=? AND owner=? ORDER BY created_at DESC, id DESC LIMIT 1`, [slug, owner]);
  return row ? statusFromRow(row) : null;
}
...
async function processSiteBuild(ctx2, id, opts) {
  ...
  for (let s = 0; s < steps; s++) {
    ...
    const blocks = arr(row.blocks);
    const keepBlock = sec.action === "keep" && typeof sec.baseIndex === "number" ? base?.blocks?.[sec.baseIndex] : void 0;
    if (keepBlock) {
      blocks.push(keepBlock);
    } else {
      const baseBlock = (typeof sec.baseIndex === "number" ? base?.blocks?.[sec.baseIndex] : void 0) ?? base?.blocks?.find((b) => b.type === sec.type) ?? null;
...
導入するには action=get に appId を渡してください（クライアントの承認後に反映されます）。`;
  ...
  if (name === "manage_site") {
    ...
    const previewHref = (s) => pubHref(s) + "?preview=1";
    if (action === "list") {
...
${lines}`;
    ...
    if (action === "status") {
      const st = await latestSiteBuildStatus(ctx2, slug, owner).catch(() => null);
      if (!st) return `「${slug}」の生成タスクは見つかりません（まだ開始していないか、完了済みです）。`;
      if (st.status === "done") return `「${slug}」のHPは完成しました（${st.total}セクション）。まだ下書き（未公開）です。下書きプレビュー：${baseUrl}${previewHref(slug)}／編集して公開：${baseUrl}/settings/site/builder?slug=${slug}（編集画面で「保存して公開」すると ${baseUrl}${pubHref(slug)} で公開されます）`;
    ...
    if (action === "build") {
      ...
      const base = await import("./sites_D1KDcV1z.mjs").then((m) => m.getSite(env, slug)).then((s) => {
        try {
...
・どうする：管理者がホーム画面の『スキル管理』で内容を確認して有効化すると、run_skill で使えるようになります。` : g.error ?? "スキル生成に失敗しました。";
    ...
    case "post_social": {
      ...
      const { socialPost } = await import("./social_cnJpe777.mjs");
    ...
    case "search_social": {
      const { socialSearch } = await import("./social_cnJpe777.mjs");
   ...
   ${i.url ?? ""}`).join("\n");
    ...
    case "read_social": {
      const { socialRead } = await import("./social_cnJpe777.mjs");
...
` + (res.gate === "ready" ? readyMsg : "→ ⛔ 問題があるため実装はブロックされました。上記の指摘を解消してから再依頼してください。");
    ...
    case "list_files": {
      ...
      const { listFilesForOwner } = await import("./storage_BN4XcC1w.mjs");
...
async function runAgent(ctx2, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_Ceeu_qGY.mjs");
  ...
  const snsOn = Object.values(await (await import("./social_cnJpe777.mjs")).socialStatus(env)).some(Boolean);
  ...
  const r2On = (await import("./storage_BN4XcC1w.mjs")).storageMode(env) === "r2";
...
${formatA2aResult(r.result)}` : `公開連絡に失敗：${r.error ?? ""}`;
    ...
    if (n === "app_render_isolation_relaxed") {
      return "ネイティブ描画（relaxed）の有効化には管理者の承認が必要です。アプリ保存時に承認申請が作成されます。";
    }
...
${PLAN_NOTE}` : "");
  ...
  const parallelSafe = (name) => name === "run_subagent" || name === "run_team";
  const out = confirmBuild ? "" : await runToolLoop(model, sysForRun, first, runDecls, exec, hops, history, onUsage, abort, opts.onEvent, answerCap, cacheBoundary, parallelSafe);
...
async function applyRenderIsolationRelaxed(ctx2, args) {
  const env = ctx2.env;
  const appId = String(args.appId ?? "");
  if (!appId) return { ok: false, result: "対象アプリが指定されていません。" };
  const allowHosts = Array.isArray(args.allowHosts) ? args.allowHosts.filter((h2) => typeof h2 === "string") : [];
  const connectHosts = Array.isArray(args.connectHosts) ? args.connectHosts.filter((h2) => typeof h2 === "string") : [];
  const stateStr = JSON.stringify({ allowHosts: [...allowHosts].sort(), connectHosts: [...connectHosts].sort() });
  let h = 0;
  for (let i = 0; i < stateStr.length; i++) h = h * 31 + stateStr.charCodeAt(i) | 0;
  const rev = Math.abs(h).toString(36);
  await env.LICENSE.put(`app_relaxed:${appId}`, JSON.stringify({ allowHosts, connectHosts, rev }));
  let applied = false;
  try {
  ...
  return { ok: true, rev, result: `ネイティブ描画（relaxed）を有効化しました（rev ${rev}${applied ? "・定義反映済" : "・次回保存で反映"}）。許可ホスト（読込）：${allowHosts.join("、") || "（なし）"}${connNote}` };
}
async function runApprovedTool(ctx2, owner, baseUrl, role, tool, args) {
  ...
  if (tool === "app_render_isolation_relaxed") {
    const r = await applyRenderIsolationRelaxed(ctx2, args);
    return r.ok ? { ok: true, result: r.result } : { ok: false, error: r.result };
  }
  const outward = await runApprovedOutwardTool(ctx2, owner, baseUrl, role, tool, args);
  if (outward === "未知のツール") return { ok: false, error: `承認されたツール「${tool}」の実装が見つかりません。運用元にご連絡ください。` };
  return { ok: true, result: outward };
}
