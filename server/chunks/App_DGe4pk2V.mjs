...
const $$App = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const [themeMod, navMod, diagMod, notifMod, clientMod] = await Promise.all([
    ...
    import("./notifications_bYzhELs1.mjs"),
    import("./client_Ceeu_qGY.mjs")
  ]);
  const [theme, navOv, limitErr, unreadNotif, entitlement, hasClaude, hasGemini, recentRaw, favRaw] = await Promise.all([
    ...
    ses && !isGuest ? import("./chat-sessions_BfrxKJEd.mjs").then((n) => n.m).then((m) => m.listSessions(ctx, ses.uid)).then((r) => r.slice(0, 6)).catch(() => []) : Promise.resolve([]),
    ses && !isGuest ? Promise.all([
      import("./settings_C4tL9AHR.mjs").then((m) => m.getFavApps(env, ses.uid)).catch(() => []),
      import("./external-apps_DRfaI5xV.mjs").then((n) => n.D).then((m) => m.installedAppDefs(ctx, ses.role)).catch(() => [])
