...
import { getToken, hostFetch, getVerifyJwk } from "./client_Ceeu_qGY.mjs";
...
import { p as preflight, i as isRunnableDefinition } from "./preflight_d8LingHU.mjs";
import { sanitizeRenderHtml, reconcileRenderScreenRefs, rewriteFabricatedDateSlots, rewriteBareCountSlots, repairMisplacedCreateTemplate, deriveConfigFieldsFromUsage, validateDefinition, isBlockedHost } from "./appdef_CnTZd11a.mjs";
import { a as isCuratedHost } from "./cdn-allowlist_rKupC5M_.mjs";
import { r as registeredParts } from "./parts_CYwgYHWx.mjs";
import { installApp } from "./apps_BKcWc3Sq.mjs";
const MAX_TAGS = 8;
...
async function fetchAndInstall(ctx, id) {
  ...
  const defObj = p.definition;
  const vr = validateDefinition(defObj);
  if (!vr.ok) return { ok: false, error: `このアプリの定義が検証に通りませんでした（${vr.issues?.[0]?.message ?? "不正な定義"}）。開発元へご連絡ください。` };
  const defJson = JSON.stringify(p.definition);
  const permJson = JSON.stringify(p.permissions ?? []);
  const prev = await ctx.db.first("SELECT definition FROM app_versions WHERE app_id=? AND version=?", [p.id, p.version]).catch(() => null);
  if (prev && (prev.definition ?? "") !== defJson) {
    return { ok: false, error: "同じバージョン番号で内容の異なるパッケージが提供されました。安全のため取込を中止しました（バージョンを上げて再申請してください）。" };
  }
  await ctx.db.batch([
    {
      sql: `INSERT INTO app_versions (app_id,version,definition,permissions,source,created_at) VALUES (?,?,?,?, 'store', ?) ON CONFLICT(app_id,version) DO NOTHING`,
      params: [p.id, p.version, defJson, permJson, now]
    },
    {
      sql: `INSERT INTO external_apps (id,name,version,category,description,permissions,definition,active_version,source,allow_fork,forked_from_id,forked_from_name,forked_from_version,installed_at) VALUES (?,?,?,?,?,?,?,?, 'store', ?,?,?,?,?)
     ON CONFLICT(id) DO UPDATE SET name=excluded.name,version=excluded.version,category=excluded.category,
      ...
      params: [p.id, p.name, p.version, p.category ?? null, p.description ?? null, permJson, defJson, p.version, p.allowFork ? 1 : 0, p.forkedFrom?.id ?? null, p.forkedFrom?.name ?? null, p.forkedFrom?.version ?? null, now]
    }
  ]);
...
async function installLocalApp(ctx, draftId, by) {
  ...
  if (d.definition) {
    try {
      const jsErr = (await import("./appdef_CnTZd11a.mjs")).checkRenderScripts(JSON.parse(d.definition).render?.html ?? "");
    ...
    try {
      ...
      if (parsed.render?.isolation === "relaxed") {
        ...
        if (okApproved) {
          ...
          const { createApproval } = await import("./approvals_DakhuEZU.mjs");
  ...
  try {
    if (d.definition) {
      const { rebuildPublicPageFromDef } = await import("./public-pages_C8bkA-DV.mjs");
...
async function deleteLocalApp(ctx, appId, deleteData = false) {
  ...
  await (await import("./public-pages_C8bkA-DV.mjs")).deletePublicPagesByApp(ctx.env, appId).catch(() => {
  });
