...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { atLeast, planLabel } = await import("./ctx_CknKdTPU.mjs").then((n) => n.M);
  const entitlement = await (await import("./client_Ceeu_qGY.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_D48l-BFG.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_BKcWc3Sq.mjs");
  const { registeredParts } = await import("./parts_CYwgYHWx.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_DRfaI5xV.mjs").then((n) => n.D);
  const { storeCatalog, myApps } = await import("./store_DfdaLbR9.mjs");
