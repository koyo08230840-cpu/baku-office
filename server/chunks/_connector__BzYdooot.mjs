...
import { DiscordInbound } from "./discord_D9MHUkhA.mjs";
import { c as checkAccess, m as markEventSeen, d as decideApprovalFromChat, j as joinViaInvite, r as respondInbound } from "./approval-chat_Cf6g75qj.mjs";
import { r as resolveUserLabels, e as extOfName, i as isSecretName, m as maskVal } from "./storage-admin_DfXGmV1d.mjs";
import { softDeleteFile, audit, inboundFileLimitBytes, buildFallbackName, saveFile } from "./storage_BN4XcC1w.mjs";
import { kvPut } from "./kv_ryUreqJI.mjs";
import { I as verifyLineSignature, J as lineLoadingStart, K as lineReply, l as linePush, L as lineReplyQuick, c as cfEgressGateway } from "./ctx_CknKdTPU.mjs";
...
import { recordInboundEvent, completeInboundEvent } from "./inbound-durable_BmD1rx_K.mjs";
...
class SlackInbound {
  ...
  async handleInbound(req, ic) {
    ...
    if (ev && !ev.bot_id && !ev.subtype && addressed) {
      ...
      if (sender && text && channel) {
        const sessionId = ev.channel_type === "im" ? `slack:im:${sender}` : `slack:ch:${channel}`;
        ic.waitUntil(
          (async () => {
            const out = await ic.respond({ connector: "slack", sender, text, channel, sessionId });
  ...
  async postMessage(channel, text) {
    const r = await this.gw.fetch("slack", "https://slack.com/api/chat.postMessage", {
      method: "POST",
...
class LineInbound {
    ...
    for (const ev of events) {
      ...
      try {
        if (m.type === "text") {
          ...
          const { enqueueUpload, uploadAck, MENU_ITEMS, WAIT_SEC, processReadyUploads } = await import("./upload-queue_CeQ0FyEU.mjs");
  ...
  async handleUploadText(userId, text, send) {
    const { resolveUploadText } = await import("./upload-queue_CeQ0FyEU.mjs");
  ...
  async fetchContent(messageId, limitBytes = 0) {
    ...
    const body = await readBodyCapped(r, limitBytes);
    if (body === "too_large") return "too_large";
    return { buf: body.buffer, mime: r.headers.get("content-type") ?? "application/octet-stream" };
  }
