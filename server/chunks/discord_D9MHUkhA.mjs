...
class DiscordInbound {
  ...
  async fetchBinary(url, mimeType) {
    ...
    const body = await readBodyCapped(r, DISCORD_MAX_FETCH_BYTES);
    if (body === "too_large") return null;
    return { mimeType: r.headers.get("content-type") ?? mimeType, dataB64: bytesToB64(body) };
  }
