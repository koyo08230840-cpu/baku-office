...
const POST = async ({ request, locals }) => {
  ...
  const { atLeast } = await import("./ctx_CknKdTPU.mjs").then((n) => n.M);
  if (!atLeast(await cachedEntitlement(env), "pro")) return json({ error: "カレンダー同期は Pro プランで利用できます。" }, 403);
  const ctx = locals.ctx;
  const LOCK = "schedule_sync_lock";
  const held = await env.LICENSE.get(LOCK).catch(() => null);
  if (held && Number(held) > nowSec()) return json({ error: "同期処理が実行中です。少し待ってから再度お試しください。" }, 409);
  await env.LICENSE.put(LOCK, String(nowSec() + 120), { expirationTtl: 130 }).catch(() => {
  });
  try {
    ...
    await env.LICENSE.delete(LOCK).catch(() => {
    });
  }
};
