...
function createSseSender(writer, enc = new TextEncoder()) {
  ...
  return {
    // SSE data 行を送る。
    send: (o) => enqueue(enc.encode(`data: ${JSON.stringify(o)}

`)),
    // コメント行（keepalive 等・data 行でないため描画に影響しない）。
    writeRaw: (s) => enqueue(enc.encode(s)),
    // 積んだ全送信を flush してから1度だけ close する（done 送信と close の競合・pending 取りこぼしを防ぐ）。
    flushAndClose: async () => {
      await chain.catch(() => {
      });
      closed = true;
...
（この会話の添付ファイル：${prev.map((p) => `${p.name || "ファイル"}[file_id=${p.fileId}${p.mime ? "・" + p.mime : ""}]`).join("、")}。利用者が「その画像」「添付のCSV」等と指す場合はこれらを対象に、file_id で読み取り・処理してください。）`;
  ...
  const sse = createSseSender(writer, enc);
  const send = (o) => sse.send(o);
  (async () => {
    try {
