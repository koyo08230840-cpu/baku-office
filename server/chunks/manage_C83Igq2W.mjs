...
import { moderate, getPublicPageAny, setPublicEnabled, setRegisterMode, setCapacity, seatsTaken, listSubmissions, deletePublicPage } from "./public-pages_C8bkA-DV.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  if (b.action === "rebuild") {
    ...
    const { rebuildPublicPage } = await import("./ctx_CknKdTPU.mjs").then((n) => n.V);
  ...
  if (b.action === "toggle") {
    ...
    if (enable) {
      ...
      if (page2) {
        const { gatePublishApp } = await import("./self-check_CtAwbKys.mjs");
        const { appendMessage } = await import("./chat-sessions_BfrxKJEd.mjs").then((n) => n.m);
