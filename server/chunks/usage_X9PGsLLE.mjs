...
async function recordCallLog(env, provider, u, meta) {
  ...
  const dur = meta.durationMs != null ? Math.max(0, Math.round(meta.durationMs)) : null;
  if (i === 0 && o === 0 && cr === 0 && cw === 0 && dur == null) return;
  try {
    await env.DB.prepare(
      "INSERT INTO ai_call_log (id, ts, provider, model, feature, plan, input_tokens, output_tokens, cache_read_tokens, cache_write_tokens, duration_ms) VALUES (?,?,?,?,?,?,?,?,?,?,?)"
    ).bind(randomId(), Math.floor(Date.now() / 1e3), provider, meta.model ?? null, meta.feature || "other", meta.plan ?? null, i, o, cr, cw, dur).run();
  } catch (e) {
