...
import { claimEventRun, endEventRun, isEventUnresolved, listTriggeredApps } from "./app-events_q7W1MIRL.mjs";
...
async function pollGoogleTrigger(env, ctx, appId, t, opts) {
  ...
  for (const item of items) {
    ...
    if (await isEventUnresolved(ctx, appId, t.id, key)) unresolved++;
  }
  if (next && next !== cursor && unresolved === 0) await setCursor(ctx, appId, t.id, next);
  return ran;
}
