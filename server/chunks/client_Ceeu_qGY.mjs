...
async function pollHost(env, deployUrl, apps) {
  ...
  try {
    ...
    if (data.entitlement === "nonprofit" || data.entitlement === "enterprise") {
      ...
      if (notified !== data.entitlement) {
        ...
        try {
          ...
          const [{ buildCtx }, notif] = await Promise.all([import("./ctx_CknKdTPU.mjs").then((n) => n.Z), import("./notifications_bYzhELs1.mjs")]);
    ...
    await (await import("./ctx_CknKdTPU.mjs").then((n) => n.U)).cacheAiKnowledge(env, data.knowledge).catch(() => {
    });
    await (await import("./ctx_CknKdTPU.mjs").then((n) => n.P)).cacheProviderRegistry(env, data.providerRegistry).catch(() => {
    });
    ...
    if (Array.isArray(data.reportUpdates) && data.reportUpdates.length) {
      const { applyReportUpdates } = await import("./reports_BaNV58AK.mjs");
