...
async function saveFile(env, file, createdBy, ctx = "org", opts) {
  ...
  if (opts?.dest === "drive") {
    const { saveBufferToDriveFolder } = await import("./drive_CKlNA6wn.mjs");
...
async function readBlob(env, ref, enc) {
  ...
  if (ref.startsWith("drive:")) {
    const { readDriveFile } = await import("./drive_CKlNA6wn.mjs");
...
async function deleteBlob(env, ref) {
  if (ref.startsWith("drive:")) {
    const { deleteDriveFile } = await import("./drive_CKlNA6wn.mjs");
