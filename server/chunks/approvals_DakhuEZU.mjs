...
async function createApproval(env, owner, tool, args, preview, meta, dbCtx) {
  ...
  try {
    const { notifyHook } = await import("./notifications_bYzhELs1.mjs");
    const msg = `🔔 承認のお願いです
${preview}
この操作を進めてよいか、下のボタンで【承認】か【却下】を選んでください（baku-office の「承認待ち」画面でも操作できます）。`;
    await notifyHook(env, msg);
    if (dbCtx) {
      const [{ notifyOwnerDirectActions }, { approvalBlocks, actionsOf }] = await Promise.all([import("./ctx_CknKdTPU.mjs").then((n) => n.T), import("./ctx_CknKdTPU.mjs").then((n) => n.O)]);
      await notifyOwnerDirectActions(dbCtx, owner, msg, actionsOf({ text: "", blocks: approvalBlocks(id) }));
    } else {
      const { notifyAdminsLine } = await import("./notifications_bYzhELs1.mjs");
