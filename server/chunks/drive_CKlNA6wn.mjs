...
import { getApiKey, saveApiKey, deleteApiKey } from "./client_Ceeu_qGY.mjs";
import { listFiles, getFile } from "./storage_BN4XcC1w.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { serviceAccountConfigured, serviceAccountAccessToken } from "./google-sa_BQsetpcp.mjs";
import { googleStatus, googleAccessToken } from "./google_cIMnQJbU.mjs";
...
async function setDriveSave(env, enabled, folder) {
  ...
  const folderName = (folder ?? cur.folder) || "baku-office";
  if (enabled) {
    const token = await driveWriteToken(env).catch(() => null);
    if (!token) return { ...cur, ok: false, error: "Google Drive への書き込み権限がありません。設定 → 連携で Drive（書込）を接続してから有効にしてください。" };
    const folderId = await ensureDriveFolder(env, folderName).catch(() => null);
    if (!folderId) return { ...cur, ok: false, error: `保存先フォルダ「${folderName}」を作成/確認できませんでした。権限とフォルダ名をご確認ください。` };
  }
  const next = { enabled: !!enabled, folder: folderName };
