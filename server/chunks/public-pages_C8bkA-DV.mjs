...
async function markSubmissionPaidBySession(env, submissionId, session) {
  ...
  const upd = await env.DB.prepare("UPDATE public_submissions SET pay_status='paid' WHERE id=? AND (pay_status IS NULL OR pay_status != 'paid')").bind(submissionId).run();
  const transitioned = (upd.meta?.changes ?? 0) > 0;
  return { ok: true, slug: row.slug, transitioned };
}
