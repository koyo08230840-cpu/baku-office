...
async function pushWebhook(gw, url, text) {
  const r = await gw.fetch("webhook", url, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ content: text, text }) }, { allowConfigured: true });
...
async function notifyAdminsLine(env, text, role = "admin") {
  const { getApiKey } = await import("./client_Ceeu_qGY.mjs");
  const token = await getApiKey(env, "line_token").catch(() => null);
  if (!token) return false;
  const targets = await linkedLineMembers(env, role);
  if (!targets.length) return false;
  const [{ cfEgressGateway }, { lineMulticast }] = await Promise.all([import("./ctx_CknKdTPU.mjs").then((n) => n.N), import("./ctx_CknKdTPU.mjs").then((n) => n.S)]);
  try {
