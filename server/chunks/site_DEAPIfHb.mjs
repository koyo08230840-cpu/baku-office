...
import { updatePageMeta, setSitePassword, clearLayout, restoreLayoutVersion, listLayoutVersions, publishLayout, saveLayoutDraft, deleteSite, upsertSite } from "./sites_D1KDcV1z.mjs";
import { v as validateLayout } from "./site-layout_IGnF6pBC.mjs";
import { audit } from "./storage_BN4XcC1w.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  const rawSlug = (b.slug ?? "").trim();
  const slugValid = /^[a-z0-9_-]{1,64}$/.test(rawSlug);
  const slug = slugValid ? rawSlug : "";
  const badSlug = () => json({ error: rawSlug ? "slug は英小文字・数字・ハイフン・アンダースコアのみ使用できます。" : "slug（英数字）が必要" }, 400);
  if (b._action === "save_page_settings") {
    if (!slug) return badSlug();
    const t = (b.title ?? "").trim();
    if (!t) return json({ error: "ページタイトルが必要です" }, 400);
    await updatePageMeta(env, slug, { title: t.slice(0, 120), show_join: !!b.show_join });
  ...
  switch (b._action) {
    ...
    case "ai_layout": {
      ...
      if (b.ref?.dataB64) {
        const { prepareDevAttachment } = await import("./chat-flow_Cx5XbqvF.mjs");
      ...
      if (Array.isArray(b.history) && b.history.length) {
        const hist = b.history.slice(-6).filter((m) => m && typeof m.text === "string" && m.text.trim()).map((m) => `${m.role === "assistant" ? "AI" : "利用者"}：${String(m.text).replace(/\s+/g, " ").trim().slice(0, 400)}`).join("\n");
        if (hist) prompt = `【これまでの会話（継続指示の文脈）】
${hist}

【今回の依頼】
${prompt || "上記の会話を踏まえて更新する。"}`;
      }
      ...
      const { startSiteBuild, processSiteBuild } = await import("./ctx_CknKdTPU.mjs").then((n) => n.W);
    ...
    case "ai_status": {
      const buildId = (b.buildId ?? "").trim();
      if (!buildId) return json({ error: "buildId が必要" }, 400);
      const { siteBuildStatus } = await import("./ctx_CknKdTPU.mjs").then((n) => n.W);
      const st = await siteBuildStatus(locals.ctx, buildId, ses.uid);
      let layout = null;
      if (st && st.status === "done") {
        const { getSite } = await import("./sites_D1KDcV1z.mjs");
        const s = await getSite(env, st.slug);
        try {
    ...
    // 公開：下書きを公開版へ反映。公開時点を版として記録（P1-22・publish 内で1回だけ）。SITE-08：空ページは拒否。
    case "publish_layout": {
      if (!slug) return badSlug();
      const pr = await publishLayout(env, slug, ses.uid);
      if (!pr.ok) return json({ error: pr.error }, 400);
      return json({ ok: true, slug });
    }
    // 公開履歴一覧（P1-22）。
    case "list_versions":
