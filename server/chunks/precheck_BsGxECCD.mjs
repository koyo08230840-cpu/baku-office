...
import { validateDefinition, refBase, AUTO_BOUND_REF_BASES } from "./appdef_CnTZd11a.mjs";
import { g as getAppDesign } from "./external-apps_DRfaI5xV.mjs";
import { p as preflight } from "./preflight_d8LingHU.mjs";
import { r as runDraftApp, p as platformInvariantSuspected, e as escalatePlatformInvariant } from "./app-runtime_DDtJx-OO.mjs";
...
function checkTransitions(def) {
  ...
  for (const u of unitsOf(def)) {
    const rawFrom = u.output?.from ?? "";
    if (!rawFrom) continue;
    const base = refBase(rawFrom);
    ...
    const names = /* @__PURE__ */ new Set([...steps.map((s) => s.as).filter(Boolean), ...u.inputs.map((i) => i.name), ...AUTO_BOUND_REF_BASES]);
    if (!names.has(base)) problems.push(`画面「${u.title}」の出力が参照する「$${base}」が見つかりません`);
  }
