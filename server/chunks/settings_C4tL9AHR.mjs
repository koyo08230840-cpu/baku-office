...
async function getBuildDriver(env) {
  if (!env.BUILD_DO) return "cron";
  const off = await memo("build_driver", 3e4, async () => await env.LICENSE.get("build_driver") === "cron", env);
  return off ? "cron" : "do";
}
async function setBuildDriver(env, useDo) {
  await kvPut(env, "build_driver", useDo ? "do" : "cron");
  invalidateMemo("build_driver", env);
}
